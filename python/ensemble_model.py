...
def ensemble_model(train_data, valid_data, test_data, name, model_list):
    ...
    train_csr_indices, train_csr_values, train_csr_shape = ti.libsvm_2_csr(train_indices, train_values)
    train_csr = csr_matrix((train_csr_values, (train_csr_indices[:, 0], train_csr_indices[:, 1])),
                           shape=train_csr_shape)
    train_labels = ti.label_2_group_id(train_labels, num_class=12)
    d_train = xgb.DMatrix(train_csr, label=train_labels)
    ...
    for model in model_list:
        for i in range(model_list[model]):
            if model == 'gblinear':
                ...
                feature_model = ti.ensemble_multi_layer_perceptron(train_data, valid_data, layer_sizes, layer_activates, opt_prop,
                ...
                max_depth = 40
                max_features = 0.1
                feature_model = ti.ensemble_rdforest(d_train, train_labels, d_valid, valid_labels, n_estimators,
